...
const QSTASH_CURRENT_KEY  = (process.env.QSTASH_CURRENT_SIGNING_KEY || "").trim();
const QSTASH_NEXT_KEY     = (process.env.QSTASH_NEXT_SIGNING_KEY    || "").trim();
const QSTASH_BASE_URL     = (process.env.QSTASH_URL || "https://qstash-eu-central-1.upstash.io").trim();
...
function getQStash() {
  if (!qstash) {
    ...
    qstash = new QStashClient({ token: QSTASH_TOKEN, baseUrl: QSTASH_BASE_URL });
  }
  return qstash;
}
...
export function getQStashReceiver() {
  if (!qstashReceiver) {
    if (!QSTASH_CURRENT_KEY || !QSTASH_NEXT_KEY) throw new Error("QStash signing keys not set.");
...
async function markDelivered(submissionId) {
  try {
    await getRedis().set(`delivered:${submissionId}`, "1", { ex: DELIVERED_TTL_SEC });
...
function retryDelaySeconds(attempt) {
  return Math.min(RETRY_MAX_DELAY_S, RETRY_BASE_DELAY_S * 2 ** Math.max(0, attempt - 1));
}
...
export async function sendToStore(store, payload) {
  ...
  if (!url) return { ok: false, permanent: true, status: 0, data: { message: `Unknown store "${store}"` } };
  ...
  try {
    ...
    let data = {};
    try { data = raw ? JSON.parse(raw) : {}; } catch (_e) { data = { raw: raw.slice(0, 300) }; }
...
export async function enqueueRetry(store, payload, submissionId, attempt) {
  if (!APP_URL) {
    console.error("[FORM_QUEUE] APP_URL not set — cannot enqueue retry. Payload:", JSON.stringify(payload));
    return false;
  }
  ...
  try {
    ...
    console.log(`[FORM_QUEUE] Queued ${store} retry #${attempt} in ${delaySec}s (id: ${submissionId})`);
    return true;
  } catch (error) {
    console.error(`[FORM_QUEUE] CRITICAL: failed to enqueue ${store} retry (${error?.message}). Payload:`, JSON.stringify(payload));
    return false;
  }
}
...
export async function submitFormToStore(store, payload) {
  ...
  if (result.permanent) {
    console.warn(`[FORM_QUEUE] ${store} permanently rejected submission (HTTP ${result.status})`);
    return result;
  }
...
export async function handleDelivery({ store, payload, submissionId, attempt }) {
  if (await isDelivered(submissionId)) {
    console.log(`[FORM_QUEUE] Retry #${attempt} skipped — already delivered (id: ${submissionId})`);
    return { ok: true, skipped: true };
  }
  ...
  if (result.ok) {
    await markDelivered(submissionId);
    console.log(`[FORM_QUEUE] Delivered ${store} on retry #${attempt} (id: ${submissionId})`);
    return { ok: true, delivered: true, attempt };
  }

  if (result.permanent) {
    console.warn(`[FORM_QUEUE] ${store} permanently rejected on retry #${attempt} — giving up.`);
    return { ok: false, permanent: true };
  }

  // Still failing — schedule next retry if attempts remain
  if (attempt >= MAX_RETRY_ATTEMPTS) {
    console.error(`[FORM_QUEUE] DEAD-LETTER: ${store} failed after ${attempt} attempts (id: ${submissionId}). Last error: ${result.error}`);
