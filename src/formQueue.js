...
//
// NOTE: Redis-backed retry queue is currently disabled — not needed right now.
// All queueing/dedup logic below is commented out. Failed sends are simply
...
// Tries to deliver now. Redis-backed retry queue is disabled, so a
// retryable failure is just logged (not queued) for now.
export async function submitFormToStore(store, payload) {
  ...
  console.error(`[FORM_QUEUE] ${store} unreachable and retry queue is disabled — LOST submission. Payload: ${JSON.stringify(payload)}`);
  return { ...result, queued: false };
}

export function initFormQueue() {
  console.warn("[FORM_QUEUE] Retry queue disabled (Redis not in use) — failed sends will be logged and lost.");
  return false;
...
export async function retryDeadLetterJobs() {
  return { enabled: false, retried: 0 };
